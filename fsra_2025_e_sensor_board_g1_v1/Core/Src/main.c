...
void CAN_Transmit(uint32_t id, uint8_t* data, uint8_t length)
{
	...
	auto x = HAL_CAN_AddTxMessage(&hcan, &TxHeader, data, &TxMailbox);

    if (x == HAL_ERROR)
    {
    	sprintf((char*)buffer,"CAN Error \n");
    }
    else if(x == HAL_BUSY)
    {
    	sprintf((char*)buffer,"CAN Busy \n");
    }
    else if(x == HAL_TIMEOUT)
    {
    	sprintf((char*)buffer,"CAN Timeout \n");
    }
    else if (x == HAL_OK)
    {
    	sprintf((char*)buffer,"CAN OK \n");
    }
...
void lsm303dlhc_read_la(uint8_t* data)
{
    ...
    x = (int16_t) (x1 * 100);
    data[0] = x & 0xFF;           // lower byte
    data[1] = (x >> 8) & 0xFF;    // higher byte

    y = (int16_t) (y1 * 100);
	data[2] = y & 0xFF;           // lower byte
	data[3] = (y >> 8) & 0xFF;    // higher byte

	z = (int16_t) (z1 * 100);
	data[4] = z & 0xFF;           // lower byte
	data[5] = (z >> 8) & 0xFF;    // higher byte
